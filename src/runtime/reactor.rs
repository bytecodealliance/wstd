//
// The p2 reactor manages WASI Pollables and converts them into async futures.
...
mod p2 {
    ...
    impl Reactor {
        ...
        pub(crate) fn pop_ready_list(&self) -> Option<Runnable> {
            self.inner.ready_list.lock().unwrap().pop_front()
        }

        pub(crate) fn ready_list_is_empty(&self) -> bool {
...
//
// The p3 reactor is a simplified task scheduler. WASI 0.3 provides native async
