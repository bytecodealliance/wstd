...
fn main() {
    ...
    find_crates(".".as_ref(), &ws, &mut crates);
...
fn read_crate(ws: Option<&Workspace>, manifest: &Path) -> Crate {
    ...
    for line in fs::read_to_string(manifest).unwrap().lines() {
        ...
        if let Some(ws) = ws {
            if version.is_none() && line.starts_with("version") && line.contains("workspace = true")
            {
...
fn publish(krate: &Crate) -> bool {
    ...
    let output = curl(&format!("https://crates.io/api/v1/crates/{}", krate.name));
    if output.is_none() {
        println!(
...
If this crate does not exist on crates.io yet please ping wstd maintainers
